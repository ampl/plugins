...
#include "ampltableconnector.hpp"
 ...
 static double
ginv(amplt::arglist *al)	/* generalized inverse of a single argument */
{
	amplt::AmplExports *ae = al->AE;
 ...
 static double
myhypot(amplt::arglist *al)	/* sqrt(x*x + y*y) */
{
 ...
 static double
mean(register amplt::arglist *al)	/* mean of arbitrarily many arguments */
{
	double x, z;
	double *d, *de, *ra;
	int *at, i, j, n;
	char *se;
	const char *sym;
	amplt::AmplExports *ae = al->AE; /* for fprintf and strtod */
