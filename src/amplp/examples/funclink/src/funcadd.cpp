...
static double
ginv(amplp::arglist *al)	/* generalized inverse of a single argument */
{
	amplp::AmplExports *ae = al->AE;
	fprintf(ae->StdErr, "Something\n");
...
static double
myhypot(amplp::arglist *al)	/* sqrt(x*x + y*y) */
{
	double *d, *h, rv, x, x0, y, y0;
...
static double
mean(register amplp::arglist *al)	/* mean of arbitrarily many arguments */
{
	double x, z;
	double *d, *de, *ra;
	int *at, i, j, n;
	char *se;
	const char *sym;
	amplp::AmplExports *ae = al->AE; /* for fprintf and strtod */
...
funcadd(amplp::AmplExports *ae){
	...
	ae->Addfunc("ginv", (ufunc*)ginv, 0, 1, 0, ae);
	ae->Addfunc("hypot", (ufunc*)myhypot, 0, 2, 0, ae);
	ae->Addfunc("mean", (ufunc*)mean, 1, -1, 0, ae);
	}
