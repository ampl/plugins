...
ginvae(arglist *al)	/* like ginv, but enrolling At_reset and At_exit */
{
	...
	if (AEInext < AEIlast) {
		AEInext->n = ++nginv;
		AEInext->ae = ae;
		ae->AtExit(ae, At_reset, AEInext);
		ae->AtReset(ae, At_end, AEInext++);
		}
	return ginv(al);
	}
 ...
 */
	...
	ae->Addfunc("ginv", (rfunc)ginv, 0, 1, 0, ae);
	ae->Addfunc("sginv", (rfunc)sginv, 2, 1, 0, ae);
	ae->Addfunc("hypot", (rfunc)myhypot, 0, 2, 0, ae);
	ae->Addfunc("ncall", (rfunc)ncall, 0, 0, 0, ae);
	ae->Addfunc("rncall", (rfunc)ncall, 4, 0, 0, ae);    /* could change 4 to 6 */
	ae->Addfunc("mean0", (rfunc)mean, 0, -1, 0, ae);
	ae->Addfunc("mean", (rfunc)mean, 1, -1, 0, ae);
	ae->Addfunc("kth", (rfunc)kth, 3, -2, 0, ae);
